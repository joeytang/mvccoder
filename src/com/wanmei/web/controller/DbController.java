...
@Controller
@RequestMapping("/db")
public class DbController  extends MvcControllerTemplate<Db,Integer,DbService>  {

	@RequestMapping(value = "/listDb")
	public void listDb(HttpServletRequest request,HttpServletResponse response,String key, ModelMap modelMap) {
		...
		List<Db> ts = baseService.list(filter, sort, 1, pageSize);
	...
	public String main(HttpServletRequest request,HttpServletResponse response, Db db,
			Integer pageNo, Integer pageSize, ModelMap modelMap) {
		return super.main(request,response, db,pageNo,pageSize, modelMap);
	}
	/**
	 * 列出所有的对象
	 * 
	...
	public String list(HttpServletRequest request,HttpServletResponse response, Db db,SortBean sort,
			Integer pageNo, Integer pageSize, ModelMap modelMap) {
		...
		try {
			...
			if(StringUtils.isNotBlank(db.getName())){
				filter.addFilter("name", db.getName(),SqlFilter.OP.like);
				searchStr.append("&name="+db.getName());
			}		
			...
			List<Db> ts = baseService.list(filter, sqlSort, pageNo, pageSize);
			int size = baseService.count(filter);
			modelMap.put(StringUtil.lowerFirstChar(className), db);
	...
	public void save(HttpServletRequest request,HttpServletResponse response, Db db,
			ModelMap modelMap) {
		super.save(request,response,db, modelMap);
	}
	@Override
	public JSONObject preSave(HttpServletRequest request, Db domain,
			ModelMap modelMap) {
	    ...
	    if(!ValidateUtils.isStringLengthValidated(domain.getName(),1,20)){
			json = RenderUtils.getStatusValidParam();
			json.put(RenderUtils.KEY_ERROR,"数据库名不能为空,且不能大于20字符！");
			return json;
	    }
	    if(!ValidateUtils.isStringLengthValidated(domain.getUserName(),1,100)){
			json = RenderUtils.getStatusValidParam();
			json.put(RenderUtils.KEY_ERROR,"用户名不能为空,且不能大于100字符！");
			return json;
	    }
	    if(!ValidateUtils.isStringLengthValidated(domain.getPassword(),1,100)){
			json = RenderUtils.getStatusValidParam();
			json.put(RenderUtils.KEY_ERROR,"密码不能为空,且不能大于100字符！");
			return json;
	    }
	    if(!ValidateUtils.isStringLengthValidated(domain.getUrl(),1,100)){
			json = RenderUtils.getStatusValidParam();
			json.put(RenderUtils.KEY_ERROR,"数据库连接地址不能为空,且不能大于100字符！");
			return json;
	    }
	    if(!ValidateUtils.isStringLengthValidated(domain.getDriver(),1,100)){
			json = RenderUtils.getStatusValidParam();
			json.put(RenderUtils.KEY_ERROR,"数据库驱动不能为空,且不能大于100字符！");
			return json;
	    }
