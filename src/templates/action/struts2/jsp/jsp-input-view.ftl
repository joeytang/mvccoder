    ...
    <s:hidden name="${domain.idName}"/>
        ...
        <#elseif (p.type.name == "timestamp")>
        	<tr>
                ...
                <td class="right">
                <s:textfield name="${p.name}" size="30" id="${p.name}Id" maxlength="50" cssClass="Wdate" cssStyle="width:180px;height:18px;" onfocus="javascript:new WdatePicker(this,'%Y-%M-%D %h:%m:%s',true);" value="%{@${project.org}.util.DateTimeUtil@forMatDate(${domain.domainName?uncap_first}.${p.name})}"/>
                    <#if (!p.isOptional()) >
                    <label class="star">*</label>
            		</#if>
                </td>
