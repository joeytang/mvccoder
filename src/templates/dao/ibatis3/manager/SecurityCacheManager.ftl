...
import ${project.org}.security.cache.MenuCache;
import ${project.org}.security.cache.ResourceCache;
...
import ${project.org}.security.domain.ResourceDetails;
...
public class SecurityCacheManager implements InitializingBean {
    ...
    private void initUserCache() throws Exception {
        List<User> users = this.userDao.listAll(-1,-1);
        if(null != users){
        	for (Iterator<User> iter = users.iterator(); iter.hasNext();) {
        		User user = iter.next();
