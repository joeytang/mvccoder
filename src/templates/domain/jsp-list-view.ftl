    ...
    <script type="text/javascript">
        ...
        function checkRemoveMore(){
		   ...
		   $.confirm("提示信息","确认删除记录",fn(b){
				if(b){
					$("#queryform").attr("action","${ctx}</#noparse>/view/${domain.domainName?uncap_first}/removeMore");
           			$("#queryform").submit();
				}else{
					return false;
				}
...
<s:form id="queryform" method="post" action="list" namespace="/security/${domain.domainName?uncap_first}">
	<div id="filterDiv" style="text-align: center;">
        ...
        <a href="<c:url value="/security/${domain.domainName?uncap_first}/input"/>">添加</a>
    </div>
    <div id="tableDiv">
        <display:table name="${domain.domainName?uncap_first}s" class="simple" id="${domain.domainName?uncap_first}" pagesize="<#noparse>$</#noparse>{pageSize}" size="<#noparse>$</#noparse>{size}" requestURI="<#noparse>$</#noparse>{ctx}/security/${domain.domainName?uncap_first}/list" partialList="true">
            ...
            <display:column title="管理">
	            <a href="<c:url value="/security/${domain.domainName?uncap_first}/input?${domain.idName}=<#noparse>$</#noparse>{${domain.domainName?uncap_first}.${domain.idName}}" />">编辑</a> &nbsp;&nbsp;&nbsp;
		        <a href="<c:url value="/security/${domain.domainName?uncap_first}/remove?${domain.idName}=<#noparse>$</#noparse>{${domain.domainName?uncap_first}.${domain.idName}}" />" onclick="JavaScript:return confirm('确认删除？');">删除</a> &nbsp;&nbsp;&nbsp;
            </display:column>
