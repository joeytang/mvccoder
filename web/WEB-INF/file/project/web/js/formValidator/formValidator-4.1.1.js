...
$.formValidator = 
{
	...
	setTipState : function(elem,showclass,showmsg)
	{
		 ...
		 */
		if(!showclass){
			if($("#elem").val() || $("#elem").val() == "0" || $.trim($("#elem").val())==""){
				showclass = "onCorrect";
			}else{
				showclass = "onShow";
			}
	...
	ajaxValid : function(returnObj)
	{
		...
		var _cele = $(initConfig.ajaxObjects);
		//alert($(_cele).attr("name"));
		//var parm = $(_cele).attr("name")+"="+$.param($(_cele).val(),true) ;
		//alert(parm);
		var parm = $(_cele).attr("name")+"="+$(_cele).val();
		/**
		 * tanghc edit--end
		 */
		//添加触发的控件名、随机数、传递的参数
