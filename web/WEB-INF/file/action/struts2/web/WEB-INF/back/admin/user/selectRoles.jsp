    ...
    <script type="text/javascript">
        ...
        function addUserRoleAuth() {
            if (confirm("确定要授权?")) {
                ...
                document.getElementById('queryform').action = '${ctx}/security/user/authRoles.html';
...
<body>
<s:form id="queryform" name="queryform" method="post" action="selectRoles" namespace="/security/user">
    ...
    <div id="filterDiv" style="text-align: center;">
        ...
        <a href="<c:url value="/security/user/list.html"/>">返回</a>&nbsp;
    ...
    <div id="tableDiv">
        <display:table name="selectRoles" class="simple" id="role">
            ...
            <display:column title="角色描述">
            	<span id="messages4${role.id}" title="cssbody=[dogvdvbdy] cssheader=[dogvdvbdy]  body=[<pre>${fn:replace(fn:replace(role.descn,']','>'),'[','<')}</pre>]"
                      style="font-family:arial;font-size:12px;font-weight:bold;color:#ABABAB;cursor:pointer">
                       ${fn:substring(role.descn, 0,10 )}</span>
            </display:column>
            ...
            <display:caption>选择用户[${name}]拥有的角色</display:caption>
        </display:table>
    </div>
</s:form>
</body>
