...
function refreshFilesList() {
	if (targetframe = window.frames['main']) {
		if (targetframe.loadcookie) {
			targetframe.loadcookie();
		}
	}
}
...
function getcookie(name) {
	...
	return cookie_start == -1 ? '' : decodeURIComponent(document.cookie
			.substring(cookie_start + name.length + 1,
	...
	if (is_ie) {
		...
		str = '<div style="text-decoration:underline;">点此复制到剪贴板</div>'
				+ AC_FL_RunContent('id', 'clipboardswf', 'name',
						'clipboardswf', 'devicefont', 'false', 'width', '100',
						'height', '20', 'src', 'images/common/clipboard.swf',
						'menu', 'false', 'allowScriptAccess', 'sameDomain',
...
function dconfirm(msg, script, width, height) {
	...
	$('floatwin_confirm_content').innerHTML = msg
			+ '<br /><button onclick="'
			+ script
			+ ';floatwin(\'close_confirm\')">&nbsp;是&nbsp;</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button onclick="floatwin(\'close_confirm\')">&nbsp;否&nbsp;</button>';
}
...
function dnotice(msg, script, width, height) {
	...
	$('floatwin_confirm_title').innerHTML = '提示信息';
	$('floatwin_confirm_content').innerHTML = msg
			+ (script ? '<br /><button onclick="' + script
					+ ';floatwin(\'close_confirm\')">确定</button>' : '');
}
...
function mb_strlen(str) {
	...
	for (var i = 0; i < str.length; i++) {
		len += str.charCodeAt(i) < 0 || str.charCodeAt(i) > 255
				? (charset == 'utf-8' ? 3 : 2)
				: 1;
	}
	return len;
...
function toggle_collapse(objname, noimg, complex, lang) {
	...
	if (obj) {
		...
		setcookie('discuz_collapse', collapsed,
				(collapsed ? 2592000 : -2592000));
	}
	if (!noimg) {
	...
	if (complex) {
		var objc = $(objname + '_c');
		objc.className = objc.className == 'c_header'
				? 'c_header closenode'
				: 'c_header';
	}
...
function sidebar_collapse(lang) {
	if (lang[0]) {
		...
		$('wrap').className = $('wrap').className == 'wrap with_side s_clear'
				? 'wrap s_clear'
...
function initCtrl(ctrlobj, click, duration, timeout, layer) {
	if (ctrlobj && !ctrlobj.initialized) {
		...
		ctrlobj.outfunc = typeof ctrlobj.onmouseout == 'function'
				? ctrlobj.onmouseout
				: null;
		ctrlobj.onmouseout = function() {
			if (this.outfunc)
				this.outfunc();
			if (duration < 3 && !jsmenu['timer'][ctrlobj.id])
				jsmenu['timer'][ctrlobj.id] = setTimeout('hideMenu(' + layer
								+ ')', timeout);
		}

		ctrlobj.overfunc = typeof ctrlobj.onmouseover == 'function'
				? ctrlobj.onmouseover
				: null;
...
function initMenu(ctrlid, menuobj, duration, timeout, layer, drag) {
	if (menuobj && !menuobj.initialized) {
		...
		if (duration < 3) {
			...
			if (duration != 1) {
				menuobj.onmouseout = function() {
					jsmenu['timer'][ctrlid] = setTimeout('hideMenu(' + layer
									+ ')', timeout);
				}
			}
		}
		...
		if (drag) {
			...
			menuobj.onmousemove = function(event) {
				try {
...
function menudrag(menuobj, e, op) {
	if (op == 1) {
		...
		var menudragnow = is_ie ? [event.clientX, event.clientY] : [e.clientX,
				e.clientY];
		menuobj.style.left = (menudragstart[2] + menudragnow[0] - menudragstart[0])
				+ 'px';
		menuobj.style.top = (menudragstart[3] + menudragnow[1] - menudragstart[1])
				+ 'px';
		doane(e);
	} else if (op == 3) {
		menudragstart = [];
...
function setMenuPosition(showid, offset) {
	...
	if (showobj) {
		...
		if ($(InFloat) != null) {
			...
			showobj.X = showobj.X - $(InFloat).scrollLeft
					- parseInt(floatwinhandle[InFloate[1] + '_1']);
			showobj.Y = showobj.Y - $(InFloat).scrollTop
					- parseInt(floatwinhandle[InFloate[1] + '_2']);
			InFloat = '';
		}
...
function Ajax(recvType, waitId) {
	...
	for (var stackId = 0; stackId < AjaxStacks.length
			&& AjaxStacks[stackId] != 0; stackId++);
		...
		if (aj.XMLHttpRequest.readyState == 4
			...
			if (aj.recvType == 'HTML') {
				...
				if (aj.XMLHttpRequest.responseXML.lastChild) {
					...
					if (ajaxdebug) {
						var error = mb_cutstr(aj.XMLHttpRequest.responseText
										.replace(/\r?\n/g, '\\n').replace(/"/g,
												'\\\"'), 200);
						aj
								.resultHandle(
										'<root>ajaxerror<script type="text/javascript" reload="1">alert(\'Ajax Error: \\n'
												+ error
												+ '\');</script></root>', aj);
					}
				}
...
function display_opacity(id, n) {
	...
	if (n >= 0) {
		n -= 10;
		$(id).style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity='
				+ n + ')';
...
function appendscript(src, text, reload, charset) {
	...
	scriptNode.id = id;
	scriptNode.charset = charset ? charset : (is_moz
...
function ajaxupdateevent(o) {
	if (typeof o == 'object' && o.getAttribute) {
		if (o.getAttribute('ajaxtarget')) {
			...
			var ajaxurl = o.getAttribute('ajaxurl')
					? o.getAttribute('ajaxurl')
					: o.href;
			_attachEvent(o, ajaxevent, newfunction('ajaxget', ajaxurl, o
									.getAttribute('ajaxtarget'), o
									.getAttribute('ajaxwaitid'), o
									.getAttribute('ajaxloading'), o
...
/*
 * @ url: 需求请求的 url @ id : 显示的 id @ waitid: 等待的 id，默认为显示的 id，如果 waitid
 * 为空字符串，则不显示 loading...， 如果为 null，则在 showid 区域显示 @ linkid: 是哪个链接触发的该 ajax
 * 请求，该对象的属性(如 ajaxdisplay)保存了一些 ajax 请求过程需要的数据。
 */
function ajaxget(url, showid, waitid, loading, display, recall) {
	...
	x.showId = $(showid);
	if (x.showId)
		x.showId.orgdisplay = typeof x.showId.orgdisplay === 'undefined'
				? x.showId.style.display
				: x.showId.orgdisplay;
...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn) {
	showloading();
	var waitid = typeof waitid == 'undefined' || waitid === null
			? showid
			: (waitid !== '' ? waitid : '');
...
function ajaxpost_load() {
	...
	try {
		...
		if (ajaxdebug) {
			var error = mb_cutstr(
					$(ajaxpostHandle[1]).contentWindow.document.body.innerText
							.replace(/\r?\n/g, '\\n').replace(/"/g, '\\\"'),
					200);
...
function ajaxmenu(e, ctrlid, timeout, func, cache, duration, ismenu, divclass,
		optionclass) {
	showloading();
	if (jsmenu['active'][0] && jsmenu['active'][0].ctrlkey == ctrlid) {
		...
		return;
			...
			&& document.readyState.toLowerCase() != 'complete') {
		return;
	}
	if (isUndefined(timeout))
		timeout = 3000;
	...
	if (isUndefined(cache))
		cache = 1;
	if (isUndefined(divclass))
		divclass = 'popupmenu_popup';
	if (isUndefined(optionclass))
		optionclass = 'popupmenu_option';
	if (isUndefined(ismenu))
		ismenu = 1;
	if (isUndefined(duration))
		duration = timeout > 0 ? 0 : 3;
	var div = $(ctrlid + '_menu');
	if (cache && div) {
		showMenu(ctrlid, e.type == 'click', 0, duration, timeout, 0, ctrlid,
				400, 1);
		if (func)
			setTimeout(func + '(' + ctrlid + ')', timeout);
		doane(e);
	} else {
		if (!div) {
		...
		var x = new Ajax();
		var href = !isUndefined($(ctrlid).href)
				? $(ctrlid).href
				: $(ctrlid).attributes['href'].value;
		x.div = div;
		x.etype = e.type;
		...
		x.get(href + '&inajax=1&ajaxmenuid=' + ctrlid + '_menu', function(s) {
					...
					if (s.indexOf('ajaxerror') != -1) {
						evalscript(s);
						evaled = true;
						if (!cache && duration != 3 && x.div.id)
							setTimeout('$("append_parent").removeChild($(\''
											+ x.div.id + '\'))', timeout);
					}
					if (!evaled
							&& (typeof ajaxerror == 'undefined' || !ajaxerror)) {
						if (x.div)
							x.div.innerHTML = '<div class="' + x.optionclass
									+ '">' + s + '</div>';
						showMenu(ctrlid, x.etype == 'click', 0, x.duration,
								x.timeout, 0, ctrlid, 400, 1);
						if (func)
							setTimeout(func + '("' + ctrlid + '")', x.timeout);
					}
					if (!evaled)
						evalscript(s);
					ajaxerror = null;
...
function pagescroll_class(obj, pagewidth, pageheight) {
	...
	this.h = function() {
		if (this.scrollpx <= this.pagewidth * this.times) {
			...
			patch = this.scrollpx > this.pagewidth * this.times ? this.scrollpx
					- this.pagewidth * this.times : 0;
			patch = patch > 0 && this.stepv < 0 ? -patch : patch;
			oldscrollLeft = this.ctrlobj.scrollLeft;
			this.ctrlobj.scrollLeft = this.ctrlobj.scrollLeft + this.stepv
					- patch;
	...
	this.v = function() {
		if (this.scrollpx <= this.pageheight * this.times) {
			this.scrollpx += Math.abs(this.stepv);
			patch = this.scrollpx > this.pageheight * this.times
					? this.scrollpx - this.pageheight * this.times
					: 0;
			patch = patch > 0 && this.stepv < 0 ? -patch : patch;
			oldscrollTop = this.ctrlobj.scrollTop;
			this.ctrlobj.scrollTop = this.ctrlobj.scrollTop + this.stepv
					- patch;
...
function loadselect(id, showinput, pageobj, pos, method) {
	...
	objoffset['width'] = is_ie ? (obj.offsetWidth
			? obj.offsetWidth
			: parseInt(obj.currentStyle.width)) : obj.offsetWidth;
	objoffset['height'] = is_ie ? (obj.offsetHeight
			? obj.offsetHeight
			: parseInt(obj.currentStyle.height)) : obj.offsetHeight;
	...
	var lis = '<ul onfocus="loadselect_keyinit(event, 1)" onblur="loadselect_keyinit(event, 2)" class="newselect" id="'
			+ objname
			+ '_selectmenu" style="'
			+ (!pos ? 'z-index:999;position: absolute; width: '
					+ objoffset['width'] + 'px;' : '') + 'display: none">';
	for (var i = 0; i < obj.options.length; i++) {
		lis += '<li '
				+ (obj.options[i].selected ? 'class="current" ' : '')
				+ 'k_id="'
				+ id
				+ '" k_value="'
				+ obj.options[i].value
				+ '" onclick="loadselect_liset(\''
				+ objname
				+ '\', '
				+ showinput
				+ ', \''
				+ id
				+ '\','
				+ (showinput ? 'this.innerHTML' : '\'' + obj.options[i].value
						+ '\'') + ',this.innerHTML, ' + i + ')">'
				+ obj.options[i].innerHTML + '</li>';
		maxlength = obj.options[i].value.length > maxlength
				? obj.options[i].value.length
				: maxlength;
	...
	lis += '</ul>';
	if (showinput) {
		inp = '<input autocomplete="off" class="newselect" id="' + objname
				+ '_selectinput" onclick="loadselect_viewmenu(this, \''
				+ objname + '\', 0, \'' + pageobj
				+ '\');doane(event)" onchange="loadselect_inputset(\'' + id
				+ '\', this.value);loadselect_viewmenu(this, \'' + objname
				+ '\', 0, \'' + pageobj + '\')" value="' + defaultopt
				+ '" style="width: ' + objoffset['width'] + 'px;height: '
				+ objoffset['height'] + 'px;" tabindex="1" />';
	} else {
		inp = '<a href="javascript:;" hidefocus="true" class="loadselect" id="'
				+ objname
				+ '_selectinput"'
				+ (!obj.disabled
						? ' onfocus="loadselect_keyinit(event, 1)" onblur="loadselect_keyinit(event, 2)" onmouseover="this.focus()" onmouseout="this.blur()" onkeyup="loadselect_key(this, event, \''
								+ objname
								+ '\', \''
								+ pageobj
								+ '\')" onclick="loadselect_viewmenu(this, \''
								+ objname
								+ '\', 0, \''
								+ pageobj
								+ '\');doane(event)"'
	...
	if (defaultopt) {
		obj.options[0] = showinput ? new Option('', defaultopt) : new Option(
				'', defaultv);
	}
...
function loadselect_keyinit(e, a) {
	if (a == 1) {
		if (document.attachEvent) {
			...
			document.body.addEventListener('keydown', loadselect_keyhandle,
					false);
		}
	} else {
		if (document.attachEvent) {
			...
			document.body.removeEventListener('keydown', loadselect_keyhandle,
					false);
		}
	}
}
	...
	if (value == 40 || value == 38) {
		...
		for (i = 0; i < lis.length; i++) {
			if (lis[i].className == 'current') {
				loadselect_liset(objname, 0, lis[i].getAttribute('k_id'),
						lis[i].getAttribute('k_value'), lis[i].innerHTML, i);
				break;
			}
		}
	}
...
function loadselect_viewmenu(ctrlobj, objname, hidden, pageobj) {
	if (!selectopen) {
		if (document.attachEvent) {
			...
			document.body.addEventListener('click', loadselect_hiddencheck,
					false);
		}
	}
	...
	if ($(objname + '_selectmenu').style.display == '' || hidden) {
		...
		if (pageobj) {
			...
			objoffset['left'] -= $(pageobj).scrollLeft
					+ parseInt(floatwinhandle[InFloate[1] + '_1']);
			objoffset['top'] -= $(pageobj).scrollTop
					+ parseInt(floatwinhandle[InFloate[1] + '_2']);
		}
...
function detectcapslock(e, obj) {
	...
	detectobj.className = (valueCapsLock >= 65 && valueCapsLock <= 90
			&& !valueShift || valueCapsLock >= 97 && valueCapsLock <= 122
			&& valueShift) ? 'capslock txt' : 'txt';
	if (is_ie) {
...
function floatwin(action, script, w, h, scrollpos) {
	...
	action = actione[0];
	if ((!allowfloatwin || allowfloatwin == 0)
			&& action == 'open'
			&& in_array(actione[1], ['register', 'login', 'newthread', 'reply',
							'edit']) && w >= 600) {
	...
	if (action == 'open') {
		...
		if (!floatwinopened) {
			...
			for (i = 0; i < objs.length; i++) {
				if (objs[i].style.visibility != 'hidden') {
					objs[i].setAttribute("oldvisibility",
		...
		if (script && script != -1) {
			...
			if (floatwinreset || floatscripthandle[scriptfile]
					&& floatscripthandle[scriptfile][0] != script) {
				if (!isUndefined(floatscripthandle[scriptfile])) {
					$('append_parent')
							.removeChild($(floatscripthandle[scriptfile][1]));
					$('append_parent')
							.removeChild($(floatscripthandle[scriptfile][1]
									+ '_mask'));
				}
		...
		if (!$(layerid)) {
			...
			div.style.left = floatwinhandle[handlekey + '_1'] = ((clientWidth - w) / 2)
					+ 'px';
			...
			$(layerid).style.top = floatwinhandle[handlekey + '_2'] = ((clientHeight - h)
					/ 2 + scrollTop)
					+ 'px';
			$(layerid).innerHTML = '<div><h3 class="float_ctrl"><em><img src="'
					+ IMGDIR
					+ '/loading.gif"> 加载中...</em><span><a href="javascript:;" class="float_close" onclick="floatwinreset = 1;floatwin(\'close_'
					+ handlekey + '\');">&nbsp</a></span></h3></div>';
			divmask = document.createElement('div');
			divmask.className = 'floatwinmask';
			divmask.id = layerid + '_mask';
			divmask.style.width = (parseInt($(layerid).style.width) + 14)
					+ 'px';
			divmask.style.height = (parseInt($(layerid).style.height) + 14)
					+ 'px';
			...
			if (script && script != -1) {
				script += (script.search(/\?/) > 0 ? '&' : '?')
						+ 'infloat=yes&handlekey=' + handlekey;
				try {
					ajaxget(script, layerid, '', '', '', scrollpos);
				} catch (e) {
					setTimeout("ajaxget('" + script + "', '" + layerid
									+ "', '', '', '', '" + scrollpos + "')",
							1000);
				}
			} else if (script == -1) {
				$(layerid).innerHTML = '<div><h3 class="float_ctrl"><em id="'
						+ layerid
						+ '_title"></em><span><a href="javascript:;" class="float_close" onclick="floatwinreset = 1;floatwin(\'close_'
						+ handlekey
						+ '\');">&nbsp</a></span></h3></div><div id="'
						+ layerid + '_content"></div>';
			...
			$(layerid).style.top = floatwinhandle[handlekey + '_2'] = ((clientHeight - h)
					/ 2 + scrollTop)
					+ 'px';
			$(layerid + '_mask').style.width = (parseInt($(layerid).style.width) + 14)
					+ 'px';
			$(layerid + '_mask').style.height = (parseInt($(layerid).style.height) + 14)
					+ 'px';
			$(layerid + '_mask').style.display = '';
			$(layerid + '_mask').style.top = (parseInt($(layerid).style.top) - 6)
					+ 'px';
		}
		...
		if (!floatwinhandle[handlekey + '_3']) {
			...
			$(layerid).style.left = floatwinhandle[handlekey + '_1'] = ((clientWidth - w) / 2)
					+ 'px';
			$(layerid).style.top = floatwinhandle[handlekey + '_2'] = ((document.documentElement.clientHeight - h)
					/ 2 + document.documentElement.scrollTop)
					+ 'px';
			$(layerid).style.width = w + 'px';
			$(layerid).style.height = h + 'px';
		} else {
			$(layerid).style.left = floatwinhandle[handlekey + '_1'] = floatwinhandle[handlekey
					+ '_3'];
			$(layerid).style.top = floatwinhandle[handlekey + '_2'] = floatwinhandle[handlekey
					+ '_4'];
		...
		$(layerid + '_mask').style.height = (parseInt($(layerid).style.height) + 14)
				+ 'px';
		$(layerid + '_mask').style.left = (parseInt($(layerid).style.left) - 6)
				+ 'px';
		$(layerid + '_mask').style.top = (parseInt($(layerid).style.top) - 6)
				+ 'px';
	}
			...
			if ($(t)) {
				...
				for (i = 0; i < lis.length; i++) {
					s += '<a href="javascript:;" onclick="$(\'' + inpid
							+ '\').value = this.innerHTML">' + lis[i].innerHTML
							+ '</a><br />';
				}
	...
	if (v >= 0) {
		if (!rettype) {
			...
			return '<a href="javascript:;" onclick="$(\''
					...
					+ '-'
					+ d.getDate()
					+ (!notime ? ' ' + d.getHours() + ':' + d.getMinutes() : '')
					+ '\'">' + s + '</a><br />';
		} else {
		...
		return '<a href="javascript:;" onclick="$(\'' + inpid + '\').focus()">'
				+ s + '</a><br />';
	} else if (v == -2) {
		return '<a href="javascript:;" onclick="$(\'' + inpid
				+ '\').onclick()">' + s + '</a><br />';
...
function creditnoticewin() {
	...
	for (i = 1; i <= 8; i++) {
		if (creditnoticearray[i] != 0 && creditinfo[i]) {
			s += '<span>'
					+ creditinfo[i][0]
					+ (creditnoticearray[i] > 0 ? '<em>+' : '<em class="desc">')
					+ creditnoticearray[i] + '</em>' + creditinfo[i][1]
					+ '</span>';
		}
	}
...
function noticewin(s, t, c) {
	...
	s = '<table cellspacing="0" cellpadding="0" class="popupcredit"><tr><td class="pc_l">&nbsp;</td><td class="pc_c"><div class="pc_inner">'
			+ s
			+ (c
					? '<a class="pc_btn" href="javascript:;" onclick="display(\'ntcwin\');setcookie(\'discuz_creditnoticedisable\', 1, 31536000);" title="不要再提示我"><img src="'
							+ IMGDIR
							+ '/popupcredit_btn.gif" alt="不要再提示我" /></a>'
	...
	pbegin = document.documentElement.scrollTop
			+ (document.documentElement.clientHeight / 2);
	pend = document.documentElement.scrollTop
			+ (document.documentElement.clientHeight / 5);
	setTimeout(function() {
				noticewin_show(pbegin, pend, 0, t)
			}, 10);
	$('ntcwin').style.left = ((document.documentElement.clientWidth - $('ntcwin').clientWidth) / 2)
			+ 'px';
...
function noticewin_show(b, e, a, t) {
	...
	if (newp > e) {
		$('ntcwin').style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity='
				+ a + ')';
		$('ntcwin').style.opacity = a / 100;
		$('ntcwin').style.top = newp + 'px';
		setTimeout(function() {
					noticewin_show(b, e, a += 10, t)
				}, 10);
	} else {
...
function toggleSite() {
	$('sitebox').style.display = $('sitebox').style.display == 'block'
			? 'none'
			: 'block';
}
...
Date.prototype.format = function(formatter) {
	...
	return formatter.replace(yearMarker, year).replace(monthMarker, month)
...
Date.parseString = function(dateString, formatter) {
	...
	var year = dateString.substring(yearPosition, yearPosition + yearLength)
			* 1;
	if (yearLength == 2) {
		if (year < 50) {
			year += 2000;
	...
	var day = dateString.substring(dayPosition, dayPosition + dayMarker.length)
			* 1;
	return new Date(year, month, day);
};

/* 提示框 */
function info($this, content, notHide, fn) {
	$('#info-box').remove();
	notHide = notHide | false;
	var $offset = $this.offset();
	var $body = $(window.document.body);
	var divDom = window.document.createElement("div");
	divDom.id = "info-box";
	divDom.className = "infobox";
	divDom.innerHTML = '<div>' + content + '</div>';
	$divDom = $(divDom);
	$body.append(divDom);
	$divDom.offset({
				left : $offset.left + ($this.width() - $divDom.width()) / 2,
				top : $offset.top + $body.scrollTop()
						+ ($this.height() - $divDom.height()) / 2
			});

	$divDom.show();
	if (!notHide)
		setTimeout(function() {
					$divDom.fadeOut(500,function(){
						if(fn){
							fn();
						}
					});
...
function loadingMask($this) {
	unloadingMask();
	var $offset = $this.offset();
	var $body = $(window.document.body);
	var imgDom = window.document.createElement("img");
	imgDom.id = "loading";
	imgDom.src = '/images/loading.gif';
	$imgDom = $(imgDom);
	$body.append(imgDom);
	$imgDom.offset({
				left : $offset.left + ($this.width() - $imgDom.width()) / 2,
				top : $offset.top + $body.scrollTop()
						+ ($this.height() - $imgDom.height()) / 2
			});

	$imgDom.show();
}
function unloadingMask(){
